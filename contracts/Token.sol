...
contract Hikiko is ERC20Capped, Ownable, UserDB, ERC20Burnable, ReentrancyGuard {
    ...
    function redeemTokens(uint256 amount, string calldata name) public onlyOwner nonReentrant {
        address x = wallets[name].wallet;
        _transfer(x, Manager, amount);
    }

    // Allow the user to fetch tokens from the contract
    function fetchTokens(uint256 amount, string calldata name) public onlyOwner nonReentrant {
        address x = wallets[name].wallet;
        transfer(x, amount);
    }

    function delete_user(string calldata name) public onlyOwner nonReentrant {
        address x = wallets[name].wallet;
    ...
    function Update_address(string calldata name, address data) public onlyOwner nonReentrant {
        address x = wallets[name].wallet;
        uint256 amount = balanceOf(x);
